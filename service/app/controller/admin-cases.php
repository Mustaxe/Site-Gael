...
/* 		if(strlen($_FILES['imagem_integra1']['name']) > 0) {
		...
		$ordem = $cases->Query("SELECT ordem
									FROM tbl_cases
		...
		$resOrdem = $cases->Query("SELECT CONCAT(ordem, ' - ', descricao) AS ordemTitulo, ordem
									FROM tbl_cases
									WHERE status = 1 AND ativo = 1
									ORDER BY ordem");
...
$app->get('/admin/cases/:id', function ($id) use ($app, $cases, $projetos, $categorias) {
    ...
    if ($res->cod == 404) {
		...
		$resOrdem = $cases->Query("SELECT CONCAT(ordem, ' - ', titulo , ' | ' , descricao) AS ordemTitulo, ordem
									FROM tbl_cases
									WHERE status = 1 AND ativo = 1
									ORDER BY ordem");
